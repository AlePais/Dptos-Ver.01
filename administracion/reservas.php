							...
							<td class="contenedor" style="margin-top:0px !important;">
								...
								<div class="filtros" >
									<form method="post" action="funciones/reserva/bd_reserva.php"  id='formulario' name='formulario'>
										...
										<p>Listado de Pagos confirmados/pendientes </p>
									<?php
										...
										while($confirmados = $resultado->fetch_array(MYSQLI_ASSOC))
										{
											...
											switch($estado)
											{
												case 'pago_confirmado';
													$select_confirmado = "selected";
													break;
												case 'pago_pendiente';
													$select_pendiente = "selected";
													break;
											}
									?>
									<p>
										...
										Estado:
										<select name="estado[<?php echo $id_reserva?>]">
											<?php
												if($estado!="reserva_cancelada")
												{
											?>
											<option value="pago_confirmado" <?php echo $select_confirmado?>>Pago Confirmado</option>
											<option value="pago_pendiente" <?php echo $select_pendiente?>>Pago Pendiente</option>
											<?php
												}
											?>
											<option value="reserva_cancelada">Cancelar Reserva</option>
										</select>
									</p>
									<?php
										}
